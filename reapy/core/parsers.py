...
class Parser:
    ...
    def parse_junk(self, junk):
        try:
            ...
            logger.exception(f'{junk["url"]} parsing failed')
...
class EstateParser(Parser):
    ...
    @nullable
    def _float(self, f):
        return float(self._float_pattern.search(f.replace(' ', '')).groups()[0])
...
class OlxEstateParser(EstateParser):
    ...
    def _parse_published(self, soup):
        ...
        return date(int(pub[2]), self._months.get(pub[1], 1), int(pub[0]))
